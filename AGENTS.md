...
- Analysis entrypoints currently include `pca_descriptive.R`, `univariate_comparison.R`, `incl_cf_univariate_comparison.R`, `correlation_plot.R`, `lasso.R`, `svm_rf_lasso_dry.R`, and `elanet.R`; all source `data_prep.R` and start from `final_ast_hc_cf_dat`.
...
- `svm_rf_lasso_dry.R` runs LASSO, SVM, and random forest classifiers in one script to check robustness of the AST-vs-HC eNose signal across classifier families and combines their performance summaries in `all_results`.
