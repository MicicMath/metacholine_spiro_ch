...
- Main setup flow: `setup.R` defines/install-loads packages and helpers, and `data_prep.R` sources setup, defines `sensors`/`cols`, imports REDCap plus eNose CSVs, adds eNose batch labels, calculates `fev1_z`/`fvc_z`, subsets to HC/asthma/CF/PCD diagnoses, and creates `final_ast_hc_cf_dat`.
- `final_ast_hc_cf_dat` is first-visit only, keeps HC plus confirmed disease (`diagnosis_status == 1`), removes non-allergic asthma, and adds `diagnosis_simple` plus `metacholine_response`; the object name is historical and PCD can now be included.
- Analysis entrypoints currently include `pca_descriptive.R`, `univariate_comparison.R`, `incl_pcd_cf_univariate_comparison.R`, `correlation_plot.R`, `lasso.R`, `svm_rf_lasso_dry.R`, and `elanet.R`; all source `data_prep.R` and start from `final_ast_hc_cf_dat`.
- `univariate_comparison.R`, `lasso.R`, and `svm_rf_lasso_dry.R` drop CF/PCD downstream and compare allergic asthma with Mid/High metacholine response against HC. `incl_pcd_cf_univariate_comparison.R` keeps CF/PCD and compares AST/CF/HC/PCD after Low/Mid/High asthma metacholine filtering.
- `pca_descriptive.R` and `correlation_plot.R` drop CF downstream and apply the Mid/High asthma metacholine filtering; PCD can remain unless explicitly filtered.
...
- `elanet.R` is an elastic-net extension/sensitivity analysis for AST classification, tuning `alpha` over `0` to `1` with the same repeated stratified 5-fold outer split structure.
- Focused smoke checks from `analysis/`: `Rscript pca_descriptive.R`, `Rscript univariate_comparison.R`, `Rscript incl_pcd_cf_univariate_comparison.R`, `Rscript correlation_plot.R`, `Rscript lasso.R`, `Rscript svm_rf_lasso_dry.R`, and `Rscript elanet.R`. Running these as `Rscript analysis/<script>.R` from the repo root will not resolve the scripts' relative paths.
