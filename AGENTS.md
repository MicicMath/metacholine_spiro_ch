...
- Analysis entrypoints currently include `pca_descriptive.R`, `univariate_comparison.R`, `incl_cf_univariate_comparison.R`, `asthma_metacholine_response_univariate.R`, `asthma_metacholine_response_hc_kruskal.R`, `correlation_plot.R`, `corr_with_spiro.R`, `lasso.R`, and `elanet.R`; all source `data_prep.R` and start from `final_ast_hc_cf_dat`.
...
- Metacholine-focused subgroup analyses have been added. `asthma_metacholine_response_univariate.R` compares asthma with negative metacholine response (`AST_Neg`) against asthma with Mid/High metacholine response (`AST_Response`) using sensor-wise Wilcoxon tests with FDR correction. `asthma_metacholine_response_hc_kruskal.R` compares HC, `AST_Neg`, and `AST_Response` using Kruskal-Wallis tests with post-hoc pairwise comparisons.
...
- `corr_with_spiro.R` is an exploratory, low-priority analysis correlating sensor readings with `fev1_z` using Pearson tests and scatterplots; it drops CF and keeps asthma with Low/Mid/High metacholine response, without retaining HC.
