...
results_elastic_net = mclapply(
  ...
  function(i) {
    ...
    alpha_auc = sapply(
      cv_models,
      function(x) {
        max(x$cvm, na.rm = TRUE)
      }
    )

    best_idx = which.max(alpha_auc)
    best_model = cv_models[[best_idx]]
    best_alpha = alphas[best_idx]

    probas = predict(best_model, newx = X_test, s = "lambda.1se", type = "response")
    predicted = ifelse(probas > 0.5, 1, 0)
    coeffs = coef(best_model, s = "lambda.1se")
    ...
    return(
      list(
        ...
        tuned_lambda = best_model$lambda.1se,
...
roc_tpr = sapply(
  seq_along(results_elastic_net),
  function(i) {
    pred = ROCR::prediction(
      as.numeric(results_elastic_net[[i]]$y_proba),
      results_elastic_net[[i]]$y_test
    )
...
# ggsave(
#   filename = "./plots/elastic_net_results.png",
#   plot = plt_final,
#   width = 24,
#   height = 12,
#   units = "cm",
